...
export default function WorkingWithArrays() {
    ...
    return (
        <div id="wd-working-with-arrays">
            ...
            <h4>3.4.7 Update Stuff</h4>
            <a href={`${API}/${todo.id}/description/${todo.description}`} className="btn btn-info float-end">
                Update Description of Todo ID = {todo.id}</a>
            
            <input value={todo.description} className="form-control w-50 float-start"
                onChange={(e) => setTodo({ ...todo, description: e.target.value })} />
            <br /><br />

            <a href={`${API}/${todo.id}/completed/${todo.completed}`} className="btn btn-info float-end">
            Complete Todo ID = {todo.id}</a>
            <input className="form-check-input me-2" type="checkbox" id="wd-todo-boxd"
